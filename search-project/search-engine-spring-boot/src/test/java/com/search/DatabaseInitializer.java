...
public class DatabaseInitializer {
    ...
    // data标题数据量
    final int dataCount = 146888;

    @Test
    public void InitSegmentTable() {
        List<String> segs = new ArrayList<>();
        // 布隆过滤器
        BloomFilter<String> bloomFilter = BloomFilter.create(Funnels.stringFunnel(Charset.forName("UTF-8")),10000000);

        // 加载停顿词
        loadStopWords(stopWordsSet, this.getClass().getResourceAsStream("/jieba/stop_words.txt"));
        {
            // 将 data 里面的 dataCount 个数据都获取到
            List<Data> datas = dataService.getSomeDatas(dataCount, 0);
            ...
            for (int i = 0; i < dataCount; i++) {
                ...
                for (SegToken segToken : segTokens) {
                    ...
                    if (word.trim().equals("") || stopWordsSet.contains(word)) continue;
                    // 布隆过滤器判断是否已经包括了
                    if (!bloomFilter.mightContain(word)) {
                        bloomFilter.put(word);   // 放进布隆过滤器
    ...
    public void initDataSegRelationTable() {
        ...
        Map<String, Integer> wordToId = new HashMap<>(dataCount);
        ...
        Map<Integer, List<DataSegment>> dataSegmentListMap = new HashMap<>(100);
        int cnt = 0;

        {
            ...
            for (int i = 0; i < dataCount; i++) {
                ...
                for (DataSegment dataSegment : segmentMap.values()) {
                    ...
                    int idx = segId % 100;
                    List<DataSegment> list = dataSegmentListMap.getOrDefault(idx, new ArrayList<>(90));
